...
int c[36]={159,243,133,253,233,228,141,18,46,195,75,52,253,204,169,30,78,94,68,11 7,4,2,33,12,2,25,195,76,26,54,98,103,205,173,65,242}; 
...
glBegin(GL_POLYGON); glVertex3f(-.74,-0.2,-0.7);//c' 
glVertex3f(-0.64,0.22,-0.7);//cc' 
glVertex3f(-0.5,0.22,-0.7);//hh'
 glVertex3f(-0.5,-0.2,-0.7);//pp' 
 glEnd(); 
...
glVertex3f(-0.5,0.22,-0.7);//hh'
 ...
 glVertex3f(0.0,-0.2,-0.7);//oo' 
 glVertex3f(-0.5,-0.2,-0.7);//pp'
 glEnd(); glBegin(GL_POLYGON);  // door2  body- near, driver 
 glVertex3f(0.12,0.22,0.7);//ll 
 glVertex3f(0.62,0.22,0.7);//kk 
...
glColor3f(0.0,0.0,1.0); // transparent objects are placed next .. 
glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); //TRANCPARENCY3 //windscreen 
glBegin(GL_POLYGON); 
...
glVertex3f(.562,.5,-.6);//AAA' 
 ...
 glVertex3f(.76,.22,.6);//MMM 
 glEnd(); 
 glBegin(GL_POLYGON);   //rear window //COLOR =WHITE TRANSPARENT 
...
void inputKey(int key, int x, int y)
 { 
    ...
    switch (key)
	{   
		case GLUT_KEY_LEFT : angle -= 0.05f;orientMe(angle);
							 break;   
		case GLUT_KEY_RIGHT : angle +=0.05f;orientMe(angle);
		                      break;
...
void movecar(int key, int x, int y)
 ...
 { 
	...
	glutAddMenuEntry("7)  a:moves Away.",1);  
